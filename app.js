...
const TURN_SPEED = 360; // turn speed in degrees per second (0 = no friction, 1 = lots of friciton!)
...
const ROIDS_VERT = 10; // average num of vertices on each asteroid
const ROIDS_JAG = 0.4; // jaggedness of the asteroids ( 0 = none )
const SHOW_BOUNDING = false; // show or hide collision bounding
const SHOW_CENTRE_DOT = false; // show or hide ship's centre dot
const SHIP_EXPLODE_DUR = .3; // duration of ship's explosion
const SHIP_INV_DUR = 3; // duration of ship's invisibility in seconds
const SHIP_BLINK_DURATION = 0.1; // duration of ship's blind during invisibility in seconds
const LASER_MAX = 10; // macimium number of lasers on screen at once
...
// set ut the game loop
setInterval(update, 1000/FPS);


function createAsteroidBelt(){
	roids = []; // clear astroids
	var x, y;
...
function keyUp(e){
	switch(e.keyCode){
		...
		case 32: // space bar (allow shooting agin)
			ship.canShoot = true;
			break;	
	}
}
...
function newAsteroid(x, y, r){
	var roid = {
		x: x,
		y: y,
		xv: Math.random() * ROIDS_SPD / FPS * (Math.random() < 0.5 ? 1 : -1), // x velocity * roids speed
		yv: Math.random() * ROIDS_SPD / FPS * (Math.random() < 0.5 ? 1 : -1),
		r: r,
		a: Math.random() * Math.PI * 2, // random angle 360* in radians
		vert: Math.floor(Math.random() * (ROIDS_VERT + 1) + ROIDS_VERT / 2),
		offs: []
	};

	// create the vertex offest array
...
function newShip(){
	return {
		...
		blinkNum: Math.ceil(SHIP_INV_DUR / SHIP_BLINK_DURATION),
		canShoot: true,
		explodeTime: 0,
		lasers: [],
		rot: 0,
...
function update(){
		var blinkOn = ship.blinkNum % 2 == 0; // even num
		...
		if(ship.thrusting){
			...
			// draw the thruster - flame behind
			if(!exploding && blinkOn){
				// when collision show no thruster
				ctx.fillStyle = 'red';
		...
		// SHIP - greenish center circle for detecting collision
		if(SHOW_BOUNDING){
		...
		for(var i = 0; i < roids.length; i++){
			...
			// ASTEROID - circles around asteroids for collision detection
			if(SHOW_BOUNDING){
				ctx.strokeStyle = 'lime';
				ctx.beginPath();
		...
		// asteroid x and y and radius... laser x and y
		var ax, ay, ar, lx, ly;
		...
		if(!exploding){ // we don't wanna move, rotate and check for collision
			if(ship.blinkNum == 0){ // if we're not currently blinking
				for(var i = 0; i < roids.length; i++){
		...
		for(var i = ship.lasers.length - 1; i >= 0; i--){
			...
			if(ship.lasers[i].explodeTime > 0){ // curently exploding
